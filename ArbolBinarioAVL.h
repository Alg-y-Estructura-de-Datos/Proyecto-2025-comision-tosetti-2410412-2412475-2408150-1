...
public:
  ArbolBinarioAVL();
 NodoArbolAVL<T>* getRaiz() const { return root; }
  void put(T data);
  ...
  void recorrer(NodoArbolAVL<T>* nodo) const {
    if (!nodo) return;

    recorrer(nodo->getLeft());

    const T& dp = nodo->getData();
    cout << left << setw(15) << dp.pais
         << setw(20) << dp.cantidadA
         << setw(20) << dp.montoA
         << setw(20) << dp.cantidadB
         << setw(20) << dp.montoB << endl;

    recorrer(nodo->getRight());
}
